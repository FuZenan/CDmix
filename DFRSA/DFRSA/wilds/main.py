...
if args.algorithm in ['CDmix', 'rsc', 'sagm']:
...
if args.algorithm not in  ['CDmix']:
...
if args.use_bert_params and args.dataset == 'civil':
    ...
    if args.algorithm == 'sam' or args.algorithm == 'CDmix_sam':
...
def split_into_groups(g):
    ...
    Returns:
        - groups (Tensor): Unique groups present in g; g 中所有唯一分组的张量
        - group_indices (list): List of Tensors, where the i-th tensor is the indices of the
                                elements of g that equal groups[i].
                                Has the same length as len(groups). 一个列表，其中每个元素是一个张量，表示 g 中等于 groups[i] 的元素的索引。
        - unique_counts (Tensor): Counts of each element in groups.
                                 Has the same length as len(groups). 每个唯一分组的计数张量。
    """
...
def train_CDmix_sam(train_loader, epoch, agg):
    running_loss = 0
    domain_loss = 0
    total_iters = len(train_loader)
    print('\n===> Epoch: {:03d}'.format(epoch))
...
def train_CDmix(train_loader, epoch, agg):
    running_loss = 0
    domain_loss = 0
    total_iters = len(train_loader)
